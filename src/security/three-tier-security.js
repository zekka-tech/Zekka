...
class ThreeTierSecurity extends EventEmitter {
  constructor(config = {}) {
    ...
    this.config = {
      ...
      twingate: {
        apiUrl: config.twingateApiUrl || process.env.TWINGATE_API_URL || 'https://api.twingate.com/v1',
      ...
      // Wazuh Configuration
      wazuh: {
        apiUrl: config.wazuhApiUrl || process.env.WAZUH_API_URL || 'https://localhost:55000',
    ...
    // Security State
    this.state = {
      tier1: { active: false, status: 'initializing', connectedUsers: 0, activeResources: 0 },
      tier2: { active: false, status: 'initializing', agents: 0, alerts: 0, threats: 0 },
      tier3: { active: false, status: 'initializing', sessions: 0, blockedIPs: [] },
    ...
    this.stats = {
      ...
      overall: { securityScore: 100, lastAssessment: new Date(), vulnerabilities: 0 }
    };
  ...
  async authenticateUser(credentials) {
    const { username, password, ip, mfaCode } = credentials;
    ...
    if (this.isIPBlocked(ip)) {
      ...
      this.logSecurityEvent('tier3.auth.blocked', { username, ip, reason: 'IP blocked' });
      return { authenticated: false, reason: 'IP blocked', locked: true };
    }
    
    // Check login attempts
    const attempts = this.loginAttempts.get(ip) || { count: 0, lastAttempt: Date.now(), locked: false };
    
    if (attempts.locked && (Date.now() - attempts.lastAttempt) < this.config.appSecurity.lockoutDuration) {
      this.stats.tier3.blockedAttempts++;
      return { authenticated: false, reason: 'Account temporarily locked', locked: true };
    }
    ...
    const validUser = username && password && password.length >= this.config.appSecurity.passwordMinLength;
    ...
    if (!validUser) {
      ...
      return { authenticated: false, reason: 'Invalid credentials', attemptsRemaining: this.config.appSecurity.maxLoginAttempts - attempts.count };
    }
    
    // MFA verification if enabled
    if (this.config.appSecurity.mfaEnabled) {
      if (!mfaCode) {
        return { authenticated: false, reason: 'MFA code required', mfaRequired: true };
      }
  ...
  validatePassword(password) {
    ...
    if (password.length < this.config.appSecurity.passwordMinLength) {
      errors.push(`Password must be at least ${this.config.appSecurity.passwordMinLength} characters`);
    }
    
    if (this.config.appSecurity.passwordRequireUppercase && !/[A-Z]/.test(password)) {
      errors.push('Password must contain at least one uppercase letter');
    }
    
    if (this.config.appSecurity.passwordRequireNumbers && !/\d/.test(password)) {
      errors.push('Password must contain at least one number');
    }
    
    if (this.config.appSecurity.passwordRequireSpecial && !/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(password)) {
  ...
  async assessSecurityPosture() {
    ...
    if (!this.state.tier1.active) {
      score -= 20;
      issues.push({ tier: 1, severity: 'high', issue: 'Network security layer not active' });
    }
    
    // Tier 2 assessment
    if (!this.state.tier2.active) {
      score -= 20;
      issues.push({ tier: 2, severity: 'high', issue: 'Threat detection layer not active' });
    }
    
    if (this.state.tier2.threats > this.config.thresholds.highSeverityAlerts) {
      score -= 15;
      issues.push({ tier: 2, severity: 'critical', issue: `${this.state.tier2.threats} unresolved threats` });
    }
    
    // Tier 3 assessment
    if (!this.state.tier3.active) {
      score -= 20;
      issues.push({ tier: 3, severity: 'high', issue: 'Application security layer not active' });
    }
    
    if (this.blockedIPs.size > 50) {
      score -= 10;
      issues.push({ tier: 3, severity: 'medium', issue: `${this.blockedIPs.size} IPs blocked` });
    }
    
    // Update stats
    this.stats.overall.securityScore = Math.max(0, score);
    this.stats.overall.lastAssessment = new Date();
    this.stats.overall.vulnerabilities = issues.filter(i => i.severity === 'critical').length;
