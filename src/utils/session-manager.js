...
class SessionManager {
  ...
  async enforceConcurrentSessionLimit(userId) {
    ...
    if (Object.keys(sessions).length > this.options.maxConcurrentSessions) {
      ...
      const sessionArray = Object.entries(sessions).map(([sessionId, data]) => ({
        sessionId,
        ...JSON.parse(data)
      })).sort((a, b) => a.createdAt - b.createdAt);
      
      // Remove oldest sessions
      const toRemove = sessionArray.slice(0, sessionArray.length - this.options.maxConcurrentSessions);
  ...
  async cleanupExpiredSessions() {
    ...
    do {
      ...
      for (const key of keys) {
        ...
        for (const [sessionId, data] of Object.entries(sessions)) {
          const sessionExists = await this.redisClient.exists(`sess:${sessionId}`);
