...
class SecurityMonitor extends EventEmitter {
  ...
  async sendAlertNotifications(alert) {
    // Email notifications
    if (
      this.options.enableEmailAlerts
      && this.options.alertEmailAddresses.length > 0
    ) {
  ...
  async getRecentAlerts(limit = 50) {
    const alertIds = await this.redisClient.lRange(
  ...
  async getMetricsHistory(hours = 24) {
    ...
    const metricsData = await this.redisClient.lRange(
      'security:metrics',
      0,
      count - 1
    );
  ...
  async detectAnomalies() {
    ...
    if (
      current.securityViolations
      > this.anomalyBaseline.securityViolations * 2
    ) {
  ...
  async getDashboardData() {
    ...
    return {
      ...
      summary: {
        ...
        unacknowledgedAlerts: recentAlerts.filter((a) => !a.acknowledged)
          .length
      }
    };
  }
