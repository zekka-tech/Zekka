...
class PasswordPolicyManager {
  ...
  validatePassword(password, userInfo = {}) {
    ...
    if (password.length > this.policy.maxLength) {
      errors.push(
        `Password must be no more than ${this.policy.maxLength} characters long`
      );
    }
    ...
    if (this.policy.requireUppercase) {
      ...
      if (uppercaseCount < this.policy.minUppercase) {
        errors.push(
          `Password must contain at least ${this.policy.minUppercase} uppercase letter(s)`
        );
      }
    }
    ...
    if (this.policy.requireLowercase) {
      ...
      if (lowercaseCount < this.policy.minLowercase) {
        errors.push(
          `Password must contain at least ${this.policy.minLowercase} lowercase letter(s)`
        );
      }
    }
    ...
    if (this.policy.requireNumbers) {
      ...
      if (numberCount < this.policy.minNumbers) {
        errors.push(
          `Password must contain at least ${this.policy.minNumbers} number(s)`
        );
      }
    }
    ...
    if (this.hasConsecutiveChars(password, this.policy.maxConsecutiveChars)) {
      errors.push(
        `Password cannot have more than ${this.policy.maxConsecutiveChars} consecutive identical characters`
      );
    }
    ...
    if (this.policy.preventUserInfo && userInfo) {
      if (this.containsUserInfo(password, userInfo)) {
        errors.push(
          'Password cannot contain personal information (name, email, etc.)'
        );
      }
    }
    ...
    if (strengthScore < this.policy.minStrengthScore) {
      errors.push(
        `Password is too weak. Strength score: ${strengthScore}/${5}. Required: ${this.policy.minStrengthScore}/${5}`
      );
    }
  ...
  async checkHistory(password, passwordHistory) {
    ...
    for (const historyEntry of passwordHistory) {
      try {
        const decryptedHash = this.keyManager.decrypt(
  ...
  shouldWarnExpiration(lastChangedAt) {
    if (
      !this.policy.expirationDays
      || !this.policy.warnDaysBefore
      || !lastChangedAt
    ) {
      return false;
    }
    ...
    return (
      ...
      && daysUntilExpiration <= this.policy.warnDaysBefore
    );
  }
