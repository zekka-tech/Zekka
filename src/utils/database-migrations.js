...
class MigrationManager {
  ...
  async rollbackMigration(filename) {
    ...
    if (typeof migration.down !== 'function') {
      throw new Error(
        `Migration ${filename} must export a 'down' function for rollback`
      );
    }
    ...
    try {
      ...
      await client.query(`DELETE FROM ${this.tableName} WHERE name = $1`, [
        filename
      ]);
...
async function runCLI() {
  ...
  try {
    ...
    case 'create':
      if (!arg) {
        console.error(
          'Migration name required: npm run migrate:create <name>'
        );
        process.exit(1);
      }
    ...
    default:
      ...
      console.log(
        '  npm run migrate:create <name> [type] - Create new migration'
      );
