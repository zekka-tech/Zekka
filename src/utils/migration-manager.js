...
class MigrationManager {
  constructor(config) {
    this.config = {
      migrationsDir: config.migrationsDir || path.join(process.cwd(), 'migrations'),
  ...
  async acquireLock(identifier = 'unknown') {
    ...
    throw new Error('Could not acquire migration lock after retries. Another migration may be in progress.');
  }
  ...
  async executeSqlMigration(filepath, direction = 'up') {
    ...
    if (!sql) {
      throw new Error(`No ${direction.toUpperCase()} section found in migration`);
    }
  ...
  async executeJsMigration(filepath, direction = 'up') {
    ...
    if (typeof migration[direction] !== 'function') {
      throw new Error(`Migration ${filepath} does not export a ${direction} function`);
    }
  ...
  async rollback(count = 1) {
    ...
    try {
      ...
      for (const executed of toRollback) {
        const migration = allMigrations.find(m => m.version === executed.version);
        if (migration) {
  ...
  async status() {
    ...
    console.log('\n📊 Migration Status\n');
    console.log('Version'.padEnd(15) + 'Name'.padEnd(40) + 'Status'.padEnd(15) + 'Executed At');
    console.log('='.repeat(100));
    
    for (const migration of allMigrations) {
      const exec = executedMap.get(migration.version);
      const status = exec 
        ? (exec.rolled_back ? '⏪ ROLLED BACK' : '✅ EXECUTED')
        : '⏳ PENDING';
    ...
    return {
      ...
      pending: allMigrations.length - executed.filter(m => !m.rolled_back).length
    };
  }
