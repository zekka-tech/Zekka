...
class Phase1DevAgents extends EventEmitter {
   ...
   */
  async initialize() {
    this.logger.info('[Phase1DevAgents] Initializing Phase 1 development agents');
  ...
  selectBestAgent(specification) {
    ...
    if (specification.type === 'full-stack') {
      return 'tempolabs';
    } else if (specification.type === 'prototype') {
      return 'bolt';
    } else if (specification.type === 'enterprise') {
      return 'softgen';
    }
  ...
  async generateWithMultiple(specification) {
    ...
    const enabledAgents = Object.keys(this.config.agents)
      .filter(agent => this.config.agents[agent].enabled);

    const promises = enabledAgents.map(agent => 
      this.generateCode(agent, specification)
        .catch(error => ({
          agent,
          error: error.message
        }))
  ...
  compareResults(results) {
    ...
    const scored = successful.map(result => ({
      agent: result.agent,
      score: result.result.metadata.linesOfCode + result.result.metadata.filesGenerated * 10,
    ...
    return {
      ...
      generations: {
        ...
        averageDuration: completed.length > 0
          ? Math.round(completed.reduce((sum, g) => sum + g.duration, 0) / completed.length)
          : 0,
