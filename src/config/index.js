...
const envSchema = joi
  .object({
    ...
    PASSWORD_MIN_LENGTH: joi.number().min(12).default(12),
    ...
    PROJECT_RATE_LIMIT_MAX: joi.number().default(10),

    // Request Size Limits
    MAX_JSON_SIZE: joi.string().default('1mb'),

    MAX_URL_ENCODED_SIZE: joi.string().default('1mb'),
    ...
    // Budget
    DAILY_BUDGET: joi.number().default(50),

    MONTHLY_BUDGET: joi.number().default(1000),

    // TwinGate & Wazuh
    TWINGATE_API_URL: joi.string().allow('').default(''),

    TWINGATE_API_KEY: joi.string().allow('').default(''),

    WAZUH_API_URL: joi.string().allow('').default(''),

    WAZUH_USERNAME: joi.string().allow('').default(''),

    WAZUH_PASSWORD: joi.string().allow('').default(''),
    ...
    DEFAULT_MODEL: joi.string().default('ollama'),

    MAX_CONCURRENT_AGENTS: joi.number().default(10),

    // Model Configuration
    ARBITRATOR_MODEL: joi.string().default('claude-sonnet-4-5'),

    ORCHESTRATOR_MODEL: joi.string().default('gemini-pro'),

    FALLBACK_MODEL: joi.string().default('llama3.1:8b'),
    ...
    GEMINI_TOP_P: joi.number().min(0).max(1).default(0.95),

    GEMINI_TOP_K: joi.number().default(40),

    GEMINI_SAFETY_HARASSMENT: joi.string().default('BLOCK_MEDIUM_AND_ABOVE'),

    GEMINI_SAFETY_HATE_SPEECH: joi.string().default('BLOCK_MEDIUM_AND_ABOVE'),

    GEMINI_SAFETY_SEXUALLY_EXPLICIT: joi
      .string()
      .default('BLOCK_MEDIUM_AND_ABOVE'),

    GEMINI_SAFETY_DANGEROUS: joi.string().default('BLOCK_MEDIUM_AND_ABOVE')
  })
...
if (error) {
  ...
  logger.error('  - JWT_SECRET: Minimum 32 characters');
  logger.error('  - ENCRYPTION_KEY: 64 hex characters (32 bytes)');
  logger.error('  - SESSION_SECRET: Minimum 32 characters');
  logger.error(
  ...
  models: {
    ...
    gemini: {
      ...
      safetySettings: {
        ...
        sexuallyExplicit:
          env.GEMINI_SAFETY_SEXUALLY_EXPLICIT || 'BLOCK_MEDIUM_AND_ABOVE',
...
logger.info(`   JWT: Configured (${config.jwt.expiration})`);
logger.info(`   Password Min Length: ${config.password.minLength}`);
logger.info(
  `   Rate Limit: ${config.rateLimit.max} req/${config.rateLimit.windowMs}ms`
);
logger.info(`   MFA: ${config.security.mfaEnabled ? 'Enabled' : 'Disabled'}`);
