...
class IntegrationTestingSuite extends EventEmitter {
  ...
  initializeTestSuites() {
    ...
    this.createSuite({
      ...
      tests: [
        ...
        },
        {
          ...
          name: 'Test workflow rollback',
          priority: 'medium'
        },
        {
          id: 'workflow-persistence',
          name: 'Test workflow state persistence',
          priority: 'high'
        }
      ]
    });
    ...
    this.createSuite({
      ...
      tests: [
        ...
        {
          id: 'agent-training',
          name: 'Test agent training workflow',
          priority: 'high'
        },
        {
    ...
    this.createSuite({
      ...
      tests: [
        {
          id: 'connection-pool',
          name: 'Test connection pooling',
          priority: 'high'
        },
        {
          id: 'transaction-handling',
          name: 'Test transaction handling',
          priority: 'high'
        },
        {
  ...
  async runSuite(suiteId) {
    ...
    try {
      ...
      console.log(
        `Suite completed: ${suite.name} (${suite.results.passed}/${suite.results.total} passed)`
      );
  ...
  async runTest(suiteId, testId) {
    ...
    let lastError = null;
    const maxAttempts = this.config.retryFailedTests
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        if (attempt > 0) {
          ...
          console.log(
            `Retrying test ${test.name}, attempt ${attempt + 1}/${maxAttempts}`
          );
  ...
  getStatistics() {
    return {
      ...
      suites: {
        ...
        completed: Array.from(this.suites.values()).filter(
          (s) => s.status === 'completed'
        ).length,
        failed: Array.from(this.suites.values()).filter(
          (s) => s.status === 'failed'
        ).length
      },
      successRate:
