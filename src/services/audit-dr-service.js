...
class AuditDRService {
  constructor(options = {}) {
    this.s3Primary = new AWS.S3({
      region:
        options.primaryRegion || process.env.AWS_PRIMARY_REGION || 'us-east-1'
    });
    this.s3Secondary = new AWS.S3({
      region:
        options.secondaryRegion
        || process.env.AWS_SECONDARY_REGION
        || 'eu-west-1'
    });

    this.bucketPrimary = options.bucketPrimary
      || process.env.S3_AUDIT_BUCKET_PRIMARY
      || 'zekka-audit-logs-us-east-1';
    this.bucketSecondary = options.bucketSecondary
      || process.env.S3_AUDIT_BUCKET_SECONDARY
      || 'zekka-audit-logs-eu-west-1';
  ...
  async setupCrossRegionReplication() {
    ...
    try {
      ...
      logger.info('✅ Cross-region replication configured for audit logs');
    } catch (error) {
      logger.error(
        '⚠️  Failed to configure cross-region replication:',
        error.message
      );
    }
  }
  ...
  async archiveAuditLogs(logDate) {
    ...
    try {
      ...
      logger.info(`✅ Audit logs archived to primary region: ${logDate}`);
      ...
      setTimeout(
        async () => {
          ...
          if (replicated) {
            logger.info(`✅ Replication verified for ${logDate}`);
          } else {
            logger.warn(
              `⚠️  Replication pending or failed for ${logDate}, initiating manual backup`
            );
            await this.uploadToS3(
              this.s3Secondary,
              this.bucketSecondary,
              logFile,
              s3Key
            );
          }
      ...
      try {
        ...
        logger.info('✅ Fallback: Audit logs archived to secondary region');
  ...
  async verifyReplication(s3Key) {
    try {
      const result = await this.s3Secondary
        .headObject({
      ...
      return (
        result.ReplicationStatus === 'COMPLETED'
        || result.ReplicationStatus === 'REPLICA'
      );
    } catch (error) {
      if (error.code === 'NotFound') {
        return false;
      }
      logger.error(`⚠️  Error verifying replication for ${s3Key}:`, error);
      return false;
    }
  }
  ...
  async restoreFromDR(date, targetRegion = 'us-east-1') {
    ...
    try {
      ...
      logger.info(`✅ Audit logs restored from ${targetRegion} for ${date}`);
      return response.Body.toString('utf-8');
    } catch (error) {
      logger.error(`❌ Failed to restore audit logs for ${date}:`, error);
      ...
      const alternateBucket = alternateRegion === 'us-east-1'
        ? this.bucketPrimary
        : this.bucketSecondary;

      try {
        logger.info(
          `🔄 Attempting restore from alternate region: ${alternateRegion}`
        );
        const response = await alternateClient
          .getObject({
        ...
        logger.info(
          `✅ Audit logs restored from alternate region ${alternateRegion} for ${date}`
        );
        return response.Body.toString('utf-8');
      } catch (alternateError) {
        throw new Error(
          `Failed to restore from both regions: ${error.message} | ${alternateError.message}`
        );
      }
    }
  }
  ...
  async getReplicationMetrics() {
    try {
      const metrics = await this.s3Primary
