...
// Common passwords to reject (top common passwords)
...
class PasswordService {
  ...
  async checkPasswordHistory(userId, password) {
    try {
      ...
      for (const row of result.rows) {
        const matches = await bcryptjs.compare(password, row.password_hash);
        if (matches) {
          return {
            reused: true,
  ...
  /**
   * Check if password change is allowed (minimum age policy)
   */
  ...
  async getPasswordExpirationReport() {
    try {
      ...
      return {
        ...
        expiringSoon: result.rows.filter((r) => r.status === 'expiring_soon').length,
