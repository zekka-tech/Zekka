...
class DevOpsPluginFramework extends EventEmitter {
  ...
  async installPlugin(pluginId, config = {}) {
    if (this.plugins.size >= this.config.maxPlugins) {
      throw new Error(
        `Maximum plugins limit reached: ${this.config.maxPlugins}`
      );
    }
  ...
  async executePlugin(pluginId, capability, params = {}) {
    ...
    if (!plugin.capabilities.includes(capability)) {
      throw new Error(
        `Plugin ${pluginId} does not support capability: ${capability}`
      );
    }
    ...
    try {
      ...
      execution.result = await this.simulateExecution(
        pluginId,
        capability,
        params
      );
  ...
  async executeHooks(hookName, context = {}) {
    ...
    for (const hook of hooks) {
      try {
        ...
        console.error(
          `Hook execution failed: ${hookName} by ${hook.pluginId}`,
          error.message
        );
      }
    }
